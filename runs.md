...
|-----|-----------|----------------|----------------------------------------|-------------------|--------|---------------|-------|
| **Reference ceiling** (R1) | none (uncompressed) | — | 4.19M | 0% | [link](logs/wikitext-103_2026-05-05_07-07-45/log.txt) | **1.2342** | Uncompressed `Linear(C, C)` at low_rank=16. The target the structured variants aim to match in BPB. |
...
| BD128 (pending) | block_diagonal | block_size=128 (16 blocks), 7.40M total | 264K | 94% | | | **Revised prediction (post-BD256):** ~58% gap recovery, BPB ~1.256 (+0.020 vs ref). Sits between BD64 (47.7%) and BD256 (67.8%) on the saturating BD curve. Worth running to confirm the curve shape, but unlikely to outperform BD256 as the production default. |
...
| BD512 (pending) | block_diagonal | block_size=512 (4 blocks), 29.42M total | 1.05M | 75% | | | **Revised prediction (post-BD256):** ~78% gap recovery, BPB ~1.246 (+0.010 vs ref). Approaches but does not match T_upper's 92.6% recovery at similar param count, suggesting BD has a structural ceiling below triangular's at high density (cross-block interactions matter more as density grows). |
| BAND32 (pending) | banded | bandwidth=32, 3.76M total | 134K | 97% | | | Pending — matches BD64 (3.73M) at matched param count. Tests whether BAND has any per-param advantage over BD at low density (where BD64 gives 47.7% gap recovery). |
...
| BAND128 (pending) | banded | bandwidth=128, 14.33M total | 512K | 88% | | | Pending — matches BD256 (14.74M) at matched param count. **The load-bearing matched-param-count comparison** for BAND vs BD at the production-relevant density. If BAND128 outperforms BD256's 67.8% recovery, BAND has a real per-param advantage even at matched params; if it ties at ~67-68%, the BAND>BD finding from BAND256 vs BD256 was an artifact of more params, not better structure. |
...
| MON32 | monarch | nblocks=32, blocks of 64x64 + perfect shuffle | 198K | 95% | [link](logs/wikitext-103_2026-05-07_08-13-16/log.txt) | **1.2612** | Recovers **47.9%** of gap, **essentially identical to BD64's 47.7% despite using 49% more lifting params (5.56M vs 3.73M)**. Per-param efficiency 0.116M / pp recovered, worse than BD64's 0.078M / pp. The Monarch parameterization (M = R·P·L·P^T with two perfect-shuffle permutations) was hypothesized to capture cross-channel interactions that pure block-diagonal misses; at this density it shows no per-param advantage over BD. |
| MON64 | monarch | nblocks=64, blocks of 32x32 + perfect shuffle | 198K | 95% | [link](logs/wikitext-103_2026-05-07_09-31-54/log.txt) | **1.2615** | Recovers **47.3%** of gap, **virtually identical to MON32 (Δ = 0.0003 BPB)**. R, L blocks total nblocks × block_size² params regardless of which is larger, so MON32 and MON64 share param count by symmetry — and now also share recovery. **Confirms Monarch's effective capacity at this density is bounded by total params, not block-shape choice.** Structural hypothesis (R·P·L·P^T captures interactions BD can't) rejected at this scale; **block-diagonal is the more efficient parameterization at low density**. |
