...
| M1r | random_topk    | 0.1%               | 4,192                                        | 0.23M (232,064)        | [link](logs/wikitext-103_2026-05-06_18-50-49/log.txt) | 1.2825 | Recovers **6.8%** of gap (vs M1's 10.6% — placement contributes ~64% as much as density at this density, absolute gap 0.0020 BPB). 0.1% is the *worst case* for random vs magnitude (low-density theoretical advantage for magnitude); the magnitude-vs-random ratio should narrow at higher densities. **Strong portability signal:** if M4r lands at 76-80% recovery, random_topk becomes the preferred default for new scales (no checkpoint dependency, near-equivalent performance). |
