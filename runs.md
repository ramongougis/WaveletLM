...
| M4r | random_topk    | 10%                | ~419,225                                     | 11.85M (11,853,016)    | [link](logs/wikitext-103_2026-05-06_22-51-34/log.txt) | 1.2560 | Recovers **57.9%** of gap (vs M4's 81.5%, +23.6 pp magnitude advantage). **Borderline-fails ±0.018 tolerance (+0.0218 vs reference 1.2361).** Magnitude-vs-random gap **widens** monotonically with density (3.8 → 10.8 → 21.1 → 23.6 pp), opposite to my earlier "narrowing" prediction. Random_topk is NOT a portable substitute for magnitude_topk at production densities — magnitude pruning is doing genuine content-aware work that random cannot replicate. |
...
| T_upper | upper_triangular | — | 2.10M (2,098,176) | 50% | [link](logs/wikitext-103_2026-05-07_00-11-31/log.txt) | **1.2381** | **Recovers 92.5% of gap, passes ±0.018 tolerance (+0.0039 vs reference 1.2342).** Lifting drops to 58.81M (-50%); total model 334.52M (-15%). |
...
| BD64 | block_diagonal | block_size=64 (32 blocks) | 131K | 97% | [link](logs/wikitext-103_2026-05-07_02-52-12/log.txt) | **1.2613** | Recovers **47.7%** of gap. At A1-equivalent param count (3.73M vs A1's 3.33M, 96.83% lifting reduction) **essentially matches the per-density recovery of magnitude_topk** at 3% (linear interp of M2/M3 ≈ 48.5%). **Best per-parameter efficiency of any variant** (0.078M lifting params / pp recovered) — outperforms M3r random_topk at 5% (43.8% recovery, 5.98M params). Doesn't pass the absolute ±0.018 tolerance (+0.0271 vs reference 1.2361), but per-parameter efficiency is the more useful metric since structural variants don't require the 2-stage training that magnitude_topk does. |
| BD128 (pending) | block_diagonal | block_size=128 (16 blocks), 7.40M total | 264K | 94% | | | Predicted: ~65-70% gap recovery, BPB ~1.252-1.255, passes ±0.018 tolerance. Fills the gap between BD64 (3% density) and BD256 (12.5%). |
| BD256 (running) | block_diagonal | block_size=256 (8 blocks), 14.74M total | 524K | 87% | | | Predicted: ~85-90% recovery, BPB ~1.244-1.249. Likely production-default candidate alongside BD128. |
| BD512 (pending) | block_diagonal | block_size=512 (4 blocks), 29.42M total | 1.05M | 75% | | | Predicted: ~92-96% recovery, BPB ~1.243-1.247. Curve's upper anchor — saturates close to reference at higher param cost. Useful data point even if not the production winner. |
