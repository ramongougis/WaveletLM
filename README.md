...
**Result (first run diverged — confounded; corrected re-run queued).** The first `disable_residual` run (07-28) NaN'd at step 5k after stalling at val ~7.46 — but that was **not** a clean residual verdict. The cause was a **second confound beyond the LR: `proj_out`'s 1e-3 "spectral epsilon" init** (a residual-regime "start-near-identity" trick). With a residual carrying the stream, the 1e-3 makes the spectral sublayer a tiny perturbation; with the residual *removed*, the spectral output **is** the signal but is starved to ~1e-3, and `ln2`'s backward (1/std on a tiny-variance input) then amplifies gradients **~100–300×/layer** → the NaN. The forward LayerNorms bound *activations*; they can't undo a backward gradient gain that size — so the run conflated "no residual" with "spectral signal numerically starved." `model.py` now **drops the 1e-3 init when `disable_residual=true`** (proj_out keeps its O(1) default), and the corrected re-run (`T5_L3_nores_v2`, top of the depth sweep) is queued. **Verdict: pending** — the earlier "residual is load-bearing" read was premature given the init confound. Expectation: the residual likely *still* proves load-bearing (it's the gradient highway, init-independent), but as a **graceful degradation** rather than a NaN — which is the result worth recording. (LR stays 0.0225 to match the residual runs; if it still NaNs near peak with the amplifier gone, *that* points to the residual/LR rather than the init. This arm also disables `per_layer_embedding`, removing every cross-layer path; whether the embedding skip alone could carry depth — residual off, embedding on — is the natural dissection follow-up.)
