...
### Single-Layer WaveletLM (L=1) at Modern Stack

Re-test L=1 with the current full feature stack (5-epoch training, 2.0× dropout, `per_scale_mixer_widths`, `cross_scale_gating`, `wavelet_crawl`, `decompose_bypass_cross_window`, refined `weight_decay`, tied embeddings) plus the parameter-reduction bundle. Most recent L=1 result is from 2026-04-11 (BPB 1.1538) and predates roughly six meaningful feature additions that contributed to the current L=2 baseline of BPB 1.0140. Headline test matrix is four runs: L=1 / L=2, each at 1 / 5 epochs (with the existing 5-epoch L=2 baseline reused as one of the four). Per-step compute roughly halves at L=1, so a 5-epoch L=1 run trains in roughly the time of a 2.5-epoch L=2 run — meaning L=1 can be trained for ~2× as many epochs as L=2 within the same compute budget. Variations to test if the headline matrix is competitive: smaller effective batch with higher learning rate, extended block size, and eventual pairing with the semantic-embedding reintroduction work.

**Interpretability motivation**: a single-layer WaveletLM has a single-hop information-flow story — every output logit traces back to the input through one composition of named operations (embedding → wavelet decompose → FWHT → gated SwiGLU → inverse FWHT → wavelet reconstruct → MLP → LM head). This makes per-scale activation patching, single-hop circuit analysis, embedding-to-coefficient mapping, and per-component ablation substantially more tractable than at L=2. Interpretability gains compound with the eventual semantic embedding reintroduction, where L=1 is the natural architectural target. The benefits are non-trivial without a semantic embedding (the learned-embedding L=1 is already easier to study than L=2) and substantial with one (both input space and architectural depth are simultaneously interpretable).

See [plans/single_layer_waveletlm.md](plans/single_layer_waveletlm.md) for the full design, the four-run test matrix with comparison logic, the variation set, composability with other post-release plans, and the BPB projection range (conservative ~1.10, mid ~1.07, optimistic ~1.04 at 5 epochs). Total new compute cost: ~10-12h on a 5090.
