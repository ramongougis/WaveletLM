...
| 3 (no residual)¶ | no-memory | n/a | 925.77M | diverged (NaN @5k) | 7.46→NaN | — | — | 5090 | [link](logs/wikitext-103_2026-06-17_07-28-30/log.txt) |
...
**Result (diverged — residual confirmed load-bearing).** The no-residual arm never trained: val stuck at ~7.46 (while the residual runs were already descending through the 4s) for ~4.7k steps, then went **NaN at step 5k** (lr≈6.4e-3, mid-warmup) and never recovered. Removing the cross-layer carry doesn't merely cost quality — depth *can't optimize* without it, so the **residual stream is the load-bearing depth mechanism** (as the transformer residual-stream literature predicts), and the flat-structure / C-as-primary-axis worry is off the table. *Caveat:* the LR (0.0225) was tuned **with** the residual, and a NaN at climbing LR is the textbook signature of an LR too high for the harder no-residual landscape — so this rigorously shows depth can't train without the residual *at the residual-tuned recipe*, not that it's impossible at any LR. The airtight version (optional) is a no-residual rerun at ~20–50× lower LR + longer warmup: if it still can't approach the residual run, the residual is *fundamentally* necessary; if it trains but worse, the residual is a powerful *optimization aid*. Either way the residual is vindicated. (Note this arm also disables `per_layer_embedding`, so it removes **every** cross-layer path; whether the embedding re-injection alone could carry depth — residual off, embedding skip on — is the natural dissection follow-up.)
