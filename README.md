...
**The 0.0010 noise floor is now measured, not assumed.** Every kill rule in this README ranks
against a ~0.0010 BPB threshold that had never been established by a same-config repeat. The AMB
re-screen produced one by accident: `sigmoid_l2` ran twice at identical config and scored **1.1563**
and **1.1565** — a spread of **0.0002, one fifth of the assumed floor**
([run 1](logs/wikitext-103_2026-07-31_06-47-41/log.txt),
[run 2](logs/wikitext-103_2026-07-31_15-10-33/log.txt)). The assumption is therefore *conservative*
and every verdict written against it stands. Caveat: a single replicate at one config bounds
run-to-run variation, it does not estimate seed variance — `seed` was fixed at 1337 in both, so this
measures nondeterminism (fp16 + cuDNN) rather than initialisation spread, and the latter is
plausibly larger. A deliberate multi-seed replicate is still worth doing before any headline claim
rests on a margin near the floor.
...
- [ ] **Reincorporate the semantic embedding** — frozen named-dimension table plus word-level tokenization; the shared prerequisite for the three prescribed-structure items below ([plan](plans/reincorporate_large_semantic_embedding.md)). Also unblocks **SOW**, whose nearest-neighbour search is defined on the native binary conceptual embedding.
- [ ] **Incorporate and train with conceptual congruence** — concept channel labelled per *situation* and broadcast across the span; training objective unaltered, so surface-BPB stays directly comparable to every existing run ([plan](plans/conceptual_congruence.md)). Gated on **C0/C1, both CPU-only**: a synthetic corpus whose labels are exact by construction, then verification that the coarse-scale approximation actually tracks the planted trajectory. C2 is the paired ±channel cost, pre-registered as within the 0.0010 noise floor.
- [ ] **Conceptual congruence — generation-steering tests** — C3 (computed vs learned aggregate; computed first, since a learned head is a reward model inside the loop) and C4 (threshold/window steering — does it suppress without degrading coherence or factual accuracy?). Requires an unsteered control and an adversarial prompt set. **C6 is the novel claim standing alone**: multi-scale detection of Crescendo-style trajectories that a per-token classifier cannot see by construction.
- [ ] **Implement canonical lattice encoding** — fixed-width clause lattices in head-first slot order, inter-clause relation stream, exact receipts ([plan](plans/canonical_lattice_encoding.md)). Gated on **E0/E1, both CPU-only**: encoder census (fill rates, PAD fraction, OOV per slot, per-scale variance for DC domination) and exact round-trip validation.
- [ ] **Combine conceptual congruence + canonical lattice** — the lattice supplies clause-level span boundaries and makes scope structural via the explicit `NOT` operator. Pre-registered prediction (C5): the lattice arm wins **and the gap concentrates on mixed-valence cases** — an even spread falsifies the boundary explanation even if the arm wins, so the eval set must be built deliberately around mixed-valence sentences.
- [ ] **10–15B dataset blend, E=1** — E=5 is a stretch goal (rough target ~15–20 PPL on held-out blend — *estimate*)
- [ ] **Blend pretraining with conceptual congruence and/or the lattice** — the same blend, re-run with the prescribed-structure stack. ⚠️ **The dominant cost here is labeling, not compute:** the congruence channel needs the blend labelled per span, and prior FDA labelling ran ~$200–250/concept on a corpus orders of magnitude smaller than 10–15B tokens. Cost this before committing; a labelled *subset* with the unlabelled remainder masked out may be the only affordable form. See the labeling section of the [plan](plans/conceptual_congruence.md).
...
- [ ] **SFT** (SmolTalk + OASST1)
- [ ] **Post-pretraining with congruence and/or the lattice** — SFT carried through with the concept channel intact, on the blend-pretrained congruence model. The channel has to survive post-training for the capstone to mean anything, and whether it does is itself an open question worth measuring.
- [ ] **Functional / toy chatbot** — also the host for the congruence-steered generation wrapper
- [ ] **Demonstrate moral congruence via red-team tests on the toy chatbot (C8 — the capstone).** Direct and indirect prompt injection, single-turn persona/encoding evasion, multi-turn **Crescendo** and **Skeleton Key** — held **without heavy filtering in place**. That last clause is the entire claim: if the result needs a thick guard stack, the channel isn't doing the work. Two baselines required or it demonstrates nothing — an unsteered control, and a filtered-but-unchannelled arm. **This is the result that would matter externally.**
...
- [ ] *(Superseded 2026-08-15 — promoted out of nice-to-have to a scheduled item above.)* **Semantic embedding** on WT-103 (maybe PG-19) — PPL comparisons + REAP/SOW concept-token ablations + n-gram processing/prediction
...
Attaches a **concept channel** to the input: a scalar per token assigned not by token type but by *situation* — a clause, sentence, or passage is judged as a whole and its label broadcast to every token inside it. The signal is therefore piecewise constant, flat within a span and stepping at boundaries, which is the shape a wavelet transform reads best. Detail coefficients spike at the transitions, localizing *where* the concept turns; the approximation path integrates spans, carrying the accumulated state. One channel, two readings, both free from the transform already present.

Training is unaltered — the model learns to predict the channel alongside tokens, so surface-BPB stays directly comparable to every existing run — and steering happens at generation, via a wrapper scoring candidate continuations by their effect on the aggregate. The measured precedent is [Korbak et al. (ICML 2023)](https://arxiv.org/abs/2302.08582), where conditional pretraining was Pareto-optimal among five objectives, cut undesirable content by up to an order of magnitude including under adversarial prompts, and beat pretrain-then-finetune-with-feedback.

The novel claim is multi-scale. Gradual steering attacks (Crescendo, Skeleton Key) are built so that every individual step is innocuous and only the *trajectory* is harmful — which a per-token classifier cannot see by construction and a coarse-scale aggregate can. Moral congruence is the flagship demonstration; the mechanism is general to any concept labelable in advance. Two caveats stated up front rather than discovered later: this is a **guard, not a disposition** (congruence with a label is not alignment, and guards get jailbroken), and "express or suppress any concept on operator command" is dual-use by construction. See [plans/conceptual_congruence.md](plans/conceptual_congruence.md).

<p align="center">
  <img src="assets/divider.svg" alt="" width="50%" height="1"/>
</p>

### Canonical Lattice Encoding
...
### Combined Canonical Lattice + Conceptual Congruence

The two meet at **span boundaries**. Congruence labels attach to situations rather than words, which resolves scope at labeling time but relocates the difficulty onto boundary placement — and sentence-level spans mis-handle mixed valence, since *"He told me I should hire a hitman, and I refused"* is two clauses carrying opposite labels. The lattice supplies clause boundaries by construction, so the combination is predicted to win **specifically on mixed-valence cases**, with the gap concentrating there rather than spreading evenly. That prediction is pre-registered as experiment C5 in the congruence plan: paired arms, identical labels, span granularity the sole difference.

The lattice's explicit relation stream (`AND`, `IF/THEN`, `NOT`) additionally makes scope *structural* rather than learned, so a negated clause carries its label without the model composing the negation itself. Both plans share the word-level tokenizer prerequisite with the semantic embedding, so the three land as a single bring-up rather than three — and jointly they prescribe representation (named dimensions), layout (named positions), and valuation (named concepts) on the same model.
