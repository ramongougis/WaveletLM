...
**Status (held):** M1 (0.1%), M2 (1%), and M3 (5%) all land at **1.2861**, identical to M0's 1.2860 across three orders of magnitude in density. The off-diagonal mass is structurally distributed (consistent with [`tools/analyze_lifting.py`](tools/analyze_lifting.py)'s near-full-rank finding, r99 ≈ 1875-1950 of 2048), not concentrated in any subset that magnitude pruning can surface. M4 (10%) and the M1r-M4r random controls remain queued, but the perfectly flat trajectory makes a step change at 10% unlikely. Validating the Lottery Ticket hypothesis here would need many more random ablations than planned, or a fundamentally different structural prior (block-diagonal, banded, monarch/butterfly, or triangular). Held pending those or a regime change. Full table in [runs.md](runs.md#wavelet-off-diagonal-masking-sweep-planned-l1-levels7-epochs1).
