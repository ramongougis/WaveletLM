...
# DEFERRED for budget (~11h; the iso-2048 L=6-vs-E=171 ranking should predict the
# iso-4096 outcome). gradient_checkpointing already wired — uncomment to run.
# run_ablation "T5_C1024_L26_iso4096d_1ep Less Width — C=1024 L=26 iso-C4096 via DEPTH (1ep, 6000)"     "$BASE_PATCH_1EP"     '{"levels": 7, "per_scale_mixer_widths": [1.0, 1.0, 1.0, 1.0, 0.5, 0.5, 0.5, 0.5], "wavelet_crawl": true, "wavelet_crawl_k": 33, "wavelet_decomp_norm": true, "wavelet_recon_norm": true, "lr": 0.05, "min_lr": 0.001, "wavelet_basis": "real", "mixer_transform": "identity", "mlp_expansion": 20, "dropout_embedding": 0.18, "dropout_projection": 0.09, "dropout_mixer": 0.09, "dropout_mlp": 0.10, "dropout_lm_head": 0.216, "weight_decay": 2e-6, "fwpkm_enabled": false, "gradient_checkpointing": true, "layers": 26, "C": 1024}'     "T5_C1024_L26_iso4096d_1ep: iso-param via DEPTH — C=1024 L=26 (~1609M) vs C=4096/L=1 (1.0963); gradient_checkpointing ON — without it 26 layers of stored activations (~6.5 GB/layer measured at L=6) would OOM the 96 GB 6000 (~180 GB)"
...
# monotonically) and the winning 455M config (C=2048/E=20) beat C=1024/E=171 with the
# SMALLER MLP. Conjecture: a wider C wants a leaner expansion (the MLP's useful capacity
# is data-set, which large C overshoots at fixed E). Drop E to 10 (MLP 336M, ~1280M
# total) and 5 (MLP 168M = EXACTLY C=2048/E=20's MLP, ~1112M total). Predict BPB
# holds-or-improves at 335-503M fewer params. All at lr=0.015 (residual stream stays
# C=4096, so the ~0.0155 mixer cliff is unchanged by MLP width) to pair with the E=20
