...
**Rejected for release.** Config flag preserved (`decompose_bypass_ema`), implementation preserved at [model.py:839](../model.py#L839) (`_compute_data_dependent_ema`) and [model.py:1038](../model.py#L1038) (block wiring). Not adopted because of a 1-epoch → 5-epoch performance inversion — see below.
...
**The inversion.** At 1 epoch, EMA beat the no-EMA reference by 0.30 nats of val loss — a large signal. At 5 epochs it regressed by +0.0025 BPB / +0.19 PPL against that same architecture's 5-epoch baseline, and is now +0.0086 BPB / +0.65 PPL behind the current best (WD=1e-6). Whatever mechanism helped at 1 epoch either inverted or was dominated by a competing regression during the remaining 4 epochs.
