...
### 12.2 `torch.compile(mode='reduce-overhead')` — **tested and rejected (2026-05-02)**

`mode='reduce-overhead'` enables CUDA Graphs capture for repeated step shapes, eliminating per-step Python and kernel launch overhead. Initially expected to be the highest-ROI quick win at `bs=16384` based on the profiler's ~18% memory-shuffle overhead reading. Tested on the L=1 / E=1 / bs=16384 / MBS=1 combined-reduction recipe (sweep run [logs/wikitext-103_2026-05-02_16-28-18/](../logs/wikitext-103_2026-05-02_16-28-18/log.txt)) and rejected for two reasons:
