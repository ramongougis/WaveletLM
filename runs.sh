...
# them in future MBS math: ~1.6-3GB per 64-batch at V=50K). Fallback engaged per plan: MBS=48
# (est ~24GB peak, ~7GB margin). If 48 ALSO OOMs, drop all rungs to 32 (lr 0.16) and restart.
# lr=0.19 = sqrt(6)-scaled from the 48/C-rule 0.094, held under the naive 0.23 (NaN cliff is
# width-bound). min_lr = lr/50. eval_interval=500 (~9.7K steps/epoch at MBS=48).
