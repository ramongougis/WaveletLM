...
# MBS-64 fit is probed immediately, per 2026-07-12 plan). Fixed C=400 / L=10, fully spectral, at
# MBS=64 (the wall-clock discount small C unlocks: C=400 used only 5.5GB-class VRAM at MBS=8, so
# 64 should fit a 32GB 5090; D0 is the cheap probe — if it OOMs in minute 1, fall back ALL rungs to
# MBS=48 (lr 0.24) or 32 (lr 0.20) and restart the ladder so rungs stay protocol-matched).
# lr=0.28 = sqrt(8)-scaled from the 48/C-rule 0.12, held under the naive 0.34 (NaN cliff is width-
# bound; C=100 ran 0.30 at MBS=64 without incident). min_lr = lr/50. eval_interval=500 (steps/epoch
# ~7.3K at MBS=64). ALL RUNGS SAME MBS+LR — internal consistency beats comparability with K3's
# MBS-8 point; D0-vs-K3 (1.0674) doubles as the batch-effect A/B at C=400 (cf. SP0-vs-K0 at C=100).
...
# deferred phi-pre shrinkage confirm later). PREDICTION ON RECORD: D2 (20ep) lands ~0.98-1.01
# sliding BPB — near SP1's 0.9805; if it ties/beats it, a 49M model matches the 239M flagship and
# the deployment story rewrites. Est (if MBS-64 ~4-5x faster): D0 ~6h/$6, D1 ~11h, D2 ~22h, D3 ~44h.
run_ablation "D0_C400_L10_fs_MBS64_5ep D-ladder 0/3 — C=400 fully spectral, MBS=64 probe + control (5ep)"     "$BASE_PATCH_5EP"     '{"levels": 7, "per_scale_mixer_widths": [1.0, 1.0, 1.0, 1.0, 0.5, 0.5, 0.5, 0.5], "wavelet_crawl": true, "wavelet_crawl_k": 33, "wavelet_decomp_norm": true, "wavelet_recon_norm": true, "lr": 0.28, "min_lr": 0.0056, "micro_batch_size": 64, "eval_interval": 500, "wavelet_basis": "real", "mixer_transform": "identity", "mlp_expansion": 0, "skip_proj_out": true, "dropout_embedding": 0.18, "dropout_projection": 0.09, "dropout_mixer": 0.09, "dropout_mlp": 0.10, "dropout_lm_head": 0.216, "weight_decay": 2e-6, "fwpkm_enabled": false, "gradient_checkpointing": false, "layers": 10, "C": 400}'     "D0_C400_L10_fs_MBS64_5ep: D-ladder control + MBS-64 VRAM probe; A/B vs K3 MBS-8 1.0674 (batch effect at C=400); if OOM fall back 48/32 + rescale lr"
run_ablation "D1_C400_L10_fs_MBS64_10ep D-ladder 1/3 — C=400 fully spectral (10ep)"     "$BASE_PATCH_5EP"     '{"epochs": 10, "levels": 7, "per_scale_mixer_widths": [1.0, 1.0, 1.0, 1.0, 0.5, 0.5, 0.5, 0.5], "wavelet_crawl": true, "wavelet_crawl_k": 33, "wavelet_decomp_norm": true, "wavelet_recon_norm": true, "lr": 0.28, "min_lr": 0.0056, "micro_batch_size": 64, "eval_interval": 500, "wavelet_basis": "real", "mixer_transform": "identity", "mlp_expansion": 0, "skip_proj_out": true, "dropout_embedding": 0.18, "dropout_projection": 0.09, "dropout_mixer": 0.09, "dropout_mlp": 0.10, "dropout_lm_head": 0.216, "weight_decay": 2e-6, "fwpkm_enabled": false, "gradient_checkpointing": false, "layers": 10, "C": 400}'     "D1_C400_L10_fs_MBS64_10ep: D-ladder E=10 (1.31B nominal tokens); watch train/val gap vs D0"
run_ablation "D2_C400_L10_fs_MBS64_20ep D-ladder 2/3 — C=400 fully spectral (20ep)"     "$BASE_PATCH_5EP"     '{"epochs": 20, "levels": 7, "per_scale_mixer_widths": [1.0, 1.0, 1.0, 1.0, 0.5, 0.5, 0.5, 0.5], "wavelet_crawl": true, "wavelet_crawl_k": 33, "wavelet_decomp_norm": true, "wavelet_recon_norm": true, "lr": 0.28, "min_lr": 0.0056, "micro_batch_size": 64, "eval_interval": 500, "wavelet_basis": "real", "mixer_transform": "identity", "mlp_expansion": 0, "skip_proj_out": true, "dropout_embedding": 0.18, "dropout_projection": 0.09, "dropout_mixer": 0.09, "dropout_mlp": 0.10, "dropout_lm_head": 0.216, "weight_decay": 2e-6, "fwpkm_enabled": false, "gradient_checkpointing": false, "layers": 10, "C": 400}'     "D2_C400_L10_fs_MBS64_20ep: D-ladder E=20 (2.62B nominal); THE prediction-scoring rung — ~0.98-1.01 est vs SP1 0.9805; ties/beats = 49M matches the flagship"
run_ablation "D3_C400_L10_fs_MBS64_40ep D-ladder 3/3 — C=400 fully spectral (40ep)"     "$BASE_PATCH_5EP"     '{"epochs": 40, "levels": 7, "per_scale_mixer_widths": [1.0, 1.0, 1.0, 1.0, 0.5, 0.5, 0.5, 0.5], "wavelet_crawl": true, "wavelet_crawl_k": 33, "wavelet_decomp_norm": true, "wavelet_recon_norm": true, "lr": 0.28, "min_lr": 0.0056, "micro_batch_size": 64, "eval_interval": 500, "wavelet_basis": "real", "mixer_transform": "identity", "mlp_expansion": 0, "skip_proj_out": true, "dropout_embedding": 0.18, "dropout_projection": 0.09, "dropout_mixer": 0.09, "dropout_mlp": 0.10, "dropout_lm_head": 0.216, "weight_decay": 2e-6, "fwpkm_enabled": false, "gradient_checkpointing": false, "layers": 10, "C": 400}'     "D3_C400_L10_fs_MBS64_40ep: D-ladder E=40 (5.24B nominal — the repeats-decay/overfit stress rung); cancellable if D2 shows the curve has flattened"
# ==============================================================================
