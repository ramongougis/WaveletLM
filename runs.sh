...
# L=5 / 5ep — the "Max from More Layers" arm and the C=2048 Medium release headline
# candidate (L3/L4 scrapped 2026-06-18; L=2/5ep already = 0.9868, sub-1.0). ACTIVE.
# ~26.9 GB on the 5090 (fits 32 GB); ~5x the L=5/1ep time. depth still paying at 5ep
# (L=2/5ep beat the old L=2 headline by 0.0272), so L=5 should land meaningfully lower.
run_ablation "T5_L5_5ep More Epochs — L=5 lean (5ep, Max/Medium headline)"     "$BASE_PATCH_5EP"     '{"levels": 7, "per_scale_mixer_widths": [1.0, 1.0, 1.0, 1.0, 0.5, 0.5, 0.5, 0.5], "wavelet_crawl": true, "wavelet_crawl_k": 33, "wavelet_decomp_norm": true, "wavelet_recon_norm": true, "lr": 0.02250, "min_lr": 0.000450, "wavelet_basis": "real", "mixer_transform": "identity", "mlp_expansion": 20, "dropout_embedding": 0.18, "dropout_projection": 0.09, "dropout_mixer": 0.09, "dropout_mlp": 0.10, "dropout_lm_head": 0.216, "weight_decay": 2e-6, "fwpkm_enabled": false, "layers": 5}'     "T5_L5_5ep: More Epochs Max arm — L=5, no-memory T5 recipe, 5 epochs; C=2048 Medium release headline candidate; ~26.9 GB on 5090"
...
# VRAM: cross-layer skip ~28-29 GB (fits the 5090); UNTIED lifting measured
# 35,821 MiB on 2026-06-18 (> the 5090's 32 GB) — 6000-only. See deferral below.
...
# DEFERRED (2026-06-18) — DO NOT RUN AS-IS (OOM + NaN). The untied ep=1 run
