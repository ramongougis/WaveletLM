...
# SUPERSEDED 2026-07-28 — DO NOT CITE THIS RUN'S BPB. It executed the OLD module
# (retrieval-only PKM, post-mixer, alpha=0 so every parameter inside was frozen) and
# measured an inert module: train AND val superimposed on the Micro baseline to +0.0001
# at step 10,000 while carrying +61.5% params and +19% wall clock. Replaced by
# FWPKM2_Micro below, which runs the actual paper. Kept visible as the negative result.
...
run_ablation "FWPKM3_Micro_C256_retention_5ep FwPKM Micro v3 - 13x retention"     "$BASE_PATCH_5EP"     '{"levels": 7, "per_scale_mixer_widths": [1.0, 1.0, 1.0, 1.0, 0.5, 0.5, 0.5, 0.5], "wavelet_crawl": true, "wavelet_crawl_k": 33, "wavelet_decomp_norm": true, "wavelet_recon_norm": true, "lr": 0.15, "min_lr": 0.003, "micro_batch_size": 48, "eval_interval": 500, "checkpoint_interval_steps": 2000, "wavelet_basis": "real", "mixer_transform": "identity", "mlp_expansion": 0, "skip_proj_out": true, "weight_decay": 2e-06, "fwpkm_enabled": true, "gradient_checkpointing": false, "layers": 10, "C": 256, "sequential_blocks": true, "dropout_embedding": 0.18, "dropout_projection": 0.09, "dropout_mixer": 0.09, "dropout_mlp": 0.1, "dropout_lm_head": 0.216, "fwpkm_num_keys": 18769, "fwpkm_top_k": 8, "fwpkm_heads": 1, "fwpkm_chunk_size": 64, "fwpkm_persist_state": true, "fwpkm_second_order": true, "fwpkm_compress_value": "zero_mean", "fwpkm_lookahead": 1, "fwpkm_lr": 1.0, "fwpkm_addr_loss_weight": 1.0, "fwpkm_idw_eps": 0.001}'     "FWPKM3 Micro: num_keys 5,625->18,769 AND top_k 32->8, together raising retention 0.69 -> 9.16 windows (~176 -> ~2,346 tokens), a 13x increase. Read FWPKM3-MINUS-FWPKM2; the width law at 75.16M demands 1.0410 and no store-scaling arm can clear it while v_init is dense. Deliberately confounds two knobs to maximise effect size on a thin budget - the disambiguation arm (5,625 keys at top_k=8, retention 2.75, ZERO extra memory) is free to run afterwards if this wins"
