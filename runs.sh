...
# D0 = practical effect. PP kill rule: PP is ~133M params (est.: +49M mixers at
# halves-widths, +11.6M lifting), so it must BEAT the width-law expectation for
# its own param class (~1.015 at MBS48, est.), not just D0 — else primes are
# dead weight bought with parameters. FT1 tests "lifting = transferable
